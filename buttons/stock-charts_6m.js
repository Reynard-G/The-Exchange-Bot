...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const data = await client.stocks.getTickData(ticker, DateTime.utc().minus({ months: 6 }).toSeconds(), DateTime.utc().toSeconds());
    const ohlc = client.utils.resampleTicksByTime(data, Duration.fromObject({ days: 7 }).minus({ minutes: 1 }).as("seconds"));
    ...
    intervalButtons.components[4].setDisabled(true);
